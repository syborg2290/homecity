...
class WriteReviewGroc extends StatefulWidget {
  ...
  @override
  _WriteReviewGrocState createState() => _WriteReviewGrocState();
}

class _WriteReviewGrocState extends State<WriteReviewGroc> {
  ...
  videoTrimDialog(String duration, File video, int index) {
    ...
    if (_review.text.trim() != "" || media.isNotEmpty) {
      ...
      if (media.isNotEmpty) {
        for (var ele in media) {
          if (ele["type"] == "image") {
            ...
            String thumbUrl = await _groceryService.uploadImageGrocThumbnail(
            ...
            String thumbUrl = await _groceryService.uploadVideoToGrocThumb(
      ...
      await _groceryService.setReviewToGrocery(
        widget.restId,
        _review.text.trim(),
        uploadMedia,
