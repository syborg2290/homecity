...
import 'package:nearby/screens/main/fetch&display/grocery/write_grocery_review.dart';
...
import 'grocery_review_reply.dart';

class GroceryReviewDisplay extends StatefulWidget {
  final String currentUserId;
  final String docId;
  final String id;
  final String grocOwnerId;
  GroceryReviewDisplay(
      {this.currentUserId, this.grocOwnerId, this.docId, this.id, Key key})
      : super(key: key);

  @override
  _GroceryReviewDisplayState createState() => _GroceryReviewDisplayState();
}

class _GroceryReviewDisplayState extends State<GroceryReviewDisplay> {
  ...
  void initState() {
    ...
    _groceryService.fetchSingleGrocery(widget.id).then((value) {
      DocumentSnapshot doc = value.documents[0];
      Grocery grocery = Grocery.fromDocument(doc);

      setState(() {
        if (grocery.reviews != null) {
          allReviews = grocery.reviews;
        }
      });
    });
              ...
              onPressed: isLoading
                  ...
                  : () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => WriteReviewGroc(
                          ...
                          child: GestureDetector(
                            onTap: () async {
                              AwesomeDialog(
                                ...
                                btnCancelText: 'No',
                                btnOkOnPress: () async {
                                  await groc.deleteGroceryReview(
                                      docId, reviewIndex);
            ...
            children: <Widget>[
              Column(
                children: <Widget>[
                  GestureDetector(
                    onTap: () async {
                      await groc.setReactionToGroceryReview(
                  ...
                  GestureDetector(
                    onTap: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => GrocReviewReplys(
