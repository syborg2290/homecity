...
class WriteGrocItemReview extends StatefulWidget {
  ...
  @override
  _WriteGrocItemReviewState createState() => _WriteGrocItemReviewState();
}

class _WriteGrocItemReviewState extends State<WriteGrocItemReview> {
  ...
  videoTrimDialog(String duration, File video, int index) {
    ...
    if (_review.text.trim() != "" || media.isNotEmpty) {
        ...
        for (var ele in media) {
          if (ele["type"] == "image") {
            String downUrl = await _grocService
                .uploadImageGroc(await compressImageFile(ele["media"], 70));
            String thumbUrl = await _grocService.uploadImageGrocThumbnail(
            ...
            String downUrl = await _grocService
                .uploadVideoToGroc(await compressVideoFile(ele["media"]));
            String thumbUrl = await _grocService.uploadVideoToGrocThumb(
      ...
      await _grocService.setReviewToGrocItem(
        widget.index,
        widget.grocId,
        _review.text.trim(),
        uploadMedia,
      ...
      if (widget.currentUserId != widget.grocOwnerId) {
