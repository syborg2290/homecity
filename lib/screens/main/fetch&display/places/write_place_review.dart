...
class WriteReviewPlace extends StatefulWidget {
  ...
  @override
  _WriteReviewPlaceState createState() => _WriteReviewPlaceState();
}

class _WriteReviewPlaceState extends State<WriteReviewPlace> {
  ...
  videoTrimDialog(String duration, File video, int index) {
    ...
    if (_review.text.trim() != "" || media.isNotEmpty) {
        ...
        for (var ele in media) {
          if (ele["type"] == "image") {
            String downUrl = await _placeService
                .uploadImagePlace(await compressImageFile(ele["media"], 70));
            String thumbUrl = await _placeService.uploadImagePlaceThumbnail(
            ...
            String downUrl = await _placeService
                .uploadVideoToPlace(await compressVideoFile(ele["media"]));
            String thumbUrl = await _placeService.uploadVideoToPlaceThumb(
      ...
      await _placeService.setReviewToPlace(
        widget.placeId,
        _review.text.trim(),
        uploadMedia,
      ...
      if (widget.currentUserId != widget.placeOwnerId) {
