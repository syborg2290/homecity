...
class ResturantItemGallery extends StatefulWidget {
  final String currentUserId;
  final String docid;
  final int index;
  final String id;
  final String restOwnerId;
  ResturantItemGallery(
      {this.currentUserId,
      this.index,
      this.docid,
      this.id,
  ...
  @override
  _ResturantItemGalleryState createState() => _ResturantItemGalleryState();
}

class _ResturantItemGalleryState extends State<ResturantItemGallery> {
  ...
  uploadMediaReply(File upload, String type) async {
    if (type == "image") {
      String downUrl = await _resturantService
          .uploadImageRest(await compressImageFile(upload, 70));
      String thumbUrl = await _resturantService
          .uploadImageRestThumbnail(await compressImageFile(upload, 40));
      var obj = {
        "url": downUrl,
        "thumb": thumbUrl,
        "type": "image",
      ...
      await _resturantService.updateItemGallery(
        widget.docid,
        widget.index,
        json.encode(obj),
      );
      ...
      String downUrl = await _resturantService
          .uploadVideoToRest(await compressVideoFile(upload));
      String thumbUrl = await _resturantService
          .uploadVideoToRestThumb(await getThumbnailForVideo(upload));
      var obj = {
        "url": downUrl,
        "thumb": thumbUrl,
        "type": "video",
      ...
      body: StreamBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (!snapshot.hasData) {
            ...
            List gallery = [];
            if (json.decode(restSnap.menu[widget.index])["gallery"] != null) {
              gallery = json.decode(restSnap.menu[widget.index])["gallery"];
            }
                      ...
                      return GestureDetector(
                        ...
                        child: Stack(
                          children: <Widget>[
                            ...
                            widget.currentUserId == widget.restOwnerId ||
                                ...
                                ? Align(
                                    ...
                                    child: GestureDetector(
                                      onTap: () async {
                                        await _resturantService
                                            .deleteRestItemGalleryMedia(index,
