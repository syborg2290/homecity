...
class AddMusic extends StatefulWidget {
  ...
  AddMusic({this.category, Key key}) : super(key: key);
                ...
                if (openController.text != "" && closeController.text != "") {
                  ...
                  if (gallery.isNotEmpty) {
                    for (var ele in gallery) {
                      if (ele["type"] == "image") {
                        String downUrl = await _musicService.uploadImage(
                            await compressImageFile(ele["media"], 80));
                        String thumbUrl =
                            await _musicService.uploadImageThumbnail(
                        ...
                        String downUrl = await _musicService
                            .uploadVideo(await compressVideoFile(ele["media"]));
                        String thumbUrl =
                            await _musicService.uploadVideoToThumb(
                    ...
                    if (sel["gallery"].isNotEmpty) {
                      for (var ele in sel["gallery"]) {
                        if (ele["type"] == "image") {
                          ...
                          String downUrl = await _musicService.uploadVideo(
                  ...
                  String initialImageUpload = await _musicService
                      .uploadImage(await compressImageFile(intialImage, 80));

                  String restId = await _musicService.addMusic(
                    currentUserId,
                    _name.text.trim(),
                    _aboutThe.text.trim(),
                    initialImageUpload,
                ...
                ? GestureDetector(
                    onTap: () async {
                      List reSllingItems = await Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => MusicMainTypes(
                                    items: sellingItems,
                                  )));
                ...
                ? GestureDetector(
                    onTap: () async {
                      List reItems = await Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => MusicRepairTypes(
                                    items: repairItems,
                                  )));
                      if (reItems != null) {
                        setState(() {
                ...
                ? GestureDetector(
                    onTap: () async {
                      List reItems = await Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => MusicRentTypes(
                                    items: rentingItems,
                                  )));
                      if (reItems != null) {
                        setState(() {
