...
import 'package:nearby/screens/main/sub/education/vocational/vocational_types.dart';
...
class AddVocational extends StatefulWidget {
  final String type;
  AddVocational({this.type, Key key}) : super(key: key);

  @override
  _AddVocationalState createState() => _AddVocationalState();
}

class _AddVocationalState extends State<AddVocational> {
  File intialImage;
      ...
      if (_name.text.trim() != "") {
        if (selectedDistrict != null) {
          if (_address.text.trim() != "") {
            if (latitude != null) {
              if (_telephone1.text != "") {
                ...
                for (var co in courses) {
                  uploadCourses.add(json.encode(co));
                }
            ...
            GradientSnackBar.showMessage(
                context, widget.type + " center" + " address is required");
          }
        } else {
        ...
        GradientSnackBar.showMessage(
            context, widget.type + " center" + " name is required");
      }
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: false,
        title: Text(
          'Add new ' + widget.type.toLowerCase() + " center",
          style: TextStyle(
      ...
      body: SingleChildScrollView(
        child: Column(
          children: <Widget>[
            ...
            textBoxContainer(
                _name,
                "* Name of the " + widget.type.toLowerCase() + " center",
                1,
                width,
                false,
                TextInputType.text),
            ...
            textBoxContainer(
                _aboutThe,
                "About the " + widget.type.toLowerCase() + " center",
                5,
                width,
                false,
                TextInputType.text),
            ...
            textBoxContainer(
                _address,
                "* Address of the " + widget.type.toLowerCase() + " center",
                1,
                width,
                false,
                TextInputType.text),
            ...
            GestureDetector(
              onTap: () async {
                List reCourse = await Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => VocationalTypes(
                              courses: courses,
                            )));
                if (reCourse != null) {
                  setState(() {
