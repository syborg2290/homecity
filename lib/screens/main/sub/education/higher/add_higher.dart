...
class AddHigherEducation extends StatefulWidget {
  final String type;

  AddHigherEducation({this.type, Key key}) : super(key: key);

  @override
  _AddHighrEducationState createState() => _AddHighrEducationState();
}

class _AddHighrEducationState extends State<AddHigherEducation> {
  File intialImage;
            ...
            GestureDetector(
              onTap: () async {
                List reCourse = await Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => HigherEducationTypes(
                              courses: courses,
                            )));
                if (reCourse != null) {
                  setState(() {
